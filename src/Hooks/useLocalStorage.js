const GetUser = () => {
  const user = localStorage.getItem(process.env.REACT_APP_USER_KEY_LOCAL);
  if (user === null || user === undefined) return false;
  const parsed = JSON.parse(user);
  if (parsed.user === null || parsed.user === undefined) return false;
  let isUndifiend =
    parsed.user.username === undefined ||
    parsed.user.token === undefined ||
    parsed.user.id === undefined;
  let isNull =
    parsed.user.username === null ||
    parsed.user.token === null ||
    parsed.user.id === null;
  if (isUndifiend && isNull) return false;
...
const AddTodo = (projectName, newTodo) => {
  let user = GetUser();
  if (!user) return;
  let project = JSON.parse(user.todo).projects.filter(
    (p) => p.name === projectName
  )[0];
  project.childs.push({ desc: newTodo, active: true });
  let otherProjects = JSON.parse(user.todo).projects.filter(
    (p) => p.name !== projectName
  );
...
const RemoveFromList = (elm, key, list) => {
  ...
  list.forEach((element) => {
    if (element[key] === elm[key]) element.active = false;
